...
Alpha = 0.05:0.05:0.5; t_repeat = 100;
Results_P = zeros(1,length(Alpha));
Results_T = zeros(2,length(Alpha));
Results_C = zeros(2,length(Alpha));
Results_D = zeros(2,length(Alpha));
Results_IA = zeros(2,length(Alpha));
for k = 1:length(Alpha)
    waitbar(0, handle_bar, ['Sim 1, alpha = ',num2str(Alpha(k))]);
    cf.alpha_2 = Alpha(k); cf.alpha_1 = cf.alpha_2/4;
...
%% Save data
save('./Results/alpha_P.mat', 'Results_P');
save('./Results/alpha_T.mat', 'Results_T');
save('./Results/alpha_C.mat', 'Results_C');
save('./Results/alpha_D.mat', 'Results_D');
save('./Results/alpha_IA.mat', 'Results_IA');
%% Plot
figure(1); hold on;
plot(Alpha,Results_P,'*-','Color',[0.85,0.33,0.1],'Linewidth',1.2);
plot(Alpha,sum(Results_T,1),'+-','Color',[0.93,0.69,0.13],'Linewidth',1.2);
plot(Alpha,sum(Results_C,1),'d-','Color',[0.49,0.18,0.56],'Linewidth',1.2);
plot(Alpha,sum(Results_D,1),'o-','Color',[0,0.45,0.74],'Linewidth',1.2);
plot(Alpha,cf.PU_constr*Results_P,'*--','Color',[0.85,0.33,0.1],'Linewidth',1.2);
plot(Alpha,Results_T(1,:),'+--','Color',[0.93,0.69,0.13],'Linewidth',1.2);
plot(Alpha,Results_C(1,:),'d--','Color',[0.49,0.18,0.56],'Linewidth',1.2);
plot(Alpha,Results_D(1,:),'o--','Color',[0,0.45,0.74],'Linewidth',1.2);
legend('Total (no SU)','Total (C-TDM)','Total (CCTS)','Total (DCTS)',...
    'PU constraint','PU (C-TDM)','PU (CCTS)','PU (DCTS)');
xlabel('Traffic density coefficient \alpha_2');
